...
def copy_call_costs(call_costs):
    return np.array([call_costs[i] for i in range(len(call_costs))])
...
def worstRemoval(call_costs, rm_size, rng):
    r = []
    L = [int(i) for i in call_costs[1]]
    while rm_size>0:
        y = rng.uniform()
        idx = int(np.power(y,3) * len(L))
        r += L[idx:idx+1]
        L = L[:idx] + L[idx+1:]
        rm_size -= 1
    return r

def update_call_costs(call_costs, sel_call, call_cost_loc, s=0):
    if type(call_cost_loc) != int:
        loc = call_cost_loc[s]
        changes = [0 for i in range(len(call_cost_loc))]
    else:
        loc = call_cost_loc
        changes = 0
    new_idx = sum(call_costs[0]>call_costs[0,loc])
    if new_idx == loc:
        return call_costs, changes
    elif new_idx < loc:
        first = np.concatenate([call_costs[:2,:new_idx] , call_costs[:2,loc:loc+1] , call_costs[:2,new_idx:loc],call_costs[:2,loc+1:]], axis = 1)
        third = call_costs[2:][0]
        third[call_costs[1,loc].astype(int)-1] = new_idx 
        third[call_costs[1,new_idx:loc].astype(int)-1] += 1
        if type(call_cost_loc) != int:
            changes = [ +1 if (c_loc>loc)&(c_loc<new_idx) else 0 for c_loc in call_cost_loc ]
        else:
            changes = 0
    else:
        first = np.concatenate([call_costs[:2,:loc] , call_costs[:2,loc+1:new_idx+1] , call_costs[:2,loc:loc+1],call_costs[:2,new_idx+1:]], axis = 1)
        third = call_costs[2:][0]
        third[call_costs[1,loc].astype(int)-1] = new_idx 
        third[call_costs[1,loc+1:new_idx+1].astype(int)-1] -= 1
        if type(call_cost_loc) != int:
            changes = [  -1 if (c_loc>loc)&(c_loc<new_idx) else 0 for c_loc in call_cost_loc ]
        else:
            changes = 0
    return np.vstack([first,third]), changes